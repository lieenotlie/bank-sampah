...
@section('page-title', 'Edit Kelurahan/Desa')
    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            <div class="card-header bg-warning text-dark">
                <h5 class="mb-0"><i class="fas fa-edit"></i> Edit Data Kelurahan/Desa</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('kelurahan.update', $kelurahan->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="kecamatan_id" class="form-select @error('kecamatan_id') is-invalid @enderror" required>
                            ...
                            @foreach($kecamatan as $item)
                                <option value="{{ $item->id }}" {{ old('kecamatan_id', $kelurahan->kecamatan_id) == $item->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jenis" class="form-select @error('jenis') is-invalid @enderror" required>
                            ...
                            <option value="Kelurahan" {{ old('jenis', $kelurahan->jenis) == 'Kelurahan' ? 'selected' : '' }}>Kelurahan</option>
                            <option value="Desa" {{ old('jenis', $kelurahan->jenis) == 'Desa' ? 'selected' : '' }}>Desa</option>
                        </select>
                        @error('jenis')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
